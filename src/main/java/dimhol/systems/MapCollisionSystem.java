...
public class MapCollisionSystem extends AbstractSystem {
     ...
     */
    private boolean checkMapCollision(final PositionComponent position, final BodyComponent body, final MovementComponent movement) {
        //Moving UP:
        if (movement.getDir().equals(new Vector2D(0, -1))) {
            ...
            return IntStream.iterate((int) x1, i -> i <= x2, i -> i + 1)
                    .anyMatch(i -> !tileMap.getTile((int) Math.floor(y), (int) Math.floor(i)).isWalkable());
        } //Moving DOWN:
            ...
            return IntStream.iterate((int) y1, i -> i <= y2, i -> i + 1)
                    .anyMatch(i -> !tileMap.getTile((int) Math.floor(i), (int) Math.floor(x)).isWalkable());
        } //Moving RIGHT:
        else if (movement.getDir().equals(new Vector2D(1, 0))) {
