...
\paragraph{Soluzione}
Ho costruito un \texttt{AISystem}, classe che estende l'\texttt{AbstractSystem}, allo scopo di processare
l'\texttt{AIComponent} e notificare al \texttt{World} nuovi eventi che i nemici possono aver creato.\\

I nemici riescono a creare eventi usando \texttt{WorldEvent}, attraverso le \texttt{Action} che 
l'\texttt{AIComponent} contiene. 
Il metodo \textit{execute()} di Action restituisce la lista (opzionale) di eventi.
...
\underline{Esempio:} in una futura versione del videogioco alcuni nemici, nel caso venissero eliminati,
...
resitituisse un \textit{Comportamento} predefinito (inteso come: Insieme di azioni,
...
a prescindere dal ordine in cui vengono inserite nuove Action nel \textit{"Comportamento"},
sono oridnate in modo tale da dare precedenza a quelle che si attivano con la minor distanza dal
giocatore; ho costruito questa classe usando il \textit{Pattern Factory Method}.\\
...
Per la creazione delle \texttt{Action} mi sono ispirato al Pattern Strategy.
...
che "coordinate" ho pensato di creare una clase \texttt{BehaviourUtil} che espone un metodo utile
