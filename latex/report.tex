...
Ci riescono grazie alle \texttt{Action} che l'\texttt{AIComponent} contiene. Tendenzialmente
un nemico avrà più Action quindi ho deciso di creare una \texttt{RoutineFactory} che mi
...
\begin{itemize}
	\item uso della libreria jts (\href[]{https://github.com/locationtech/jts}{link alla pagina github}) sia per la classe \texttt{Vector2D} che e' usata in tutto il progetto, sia per per creare facilmente forme geometriche di interfaccia \texttt{Polygon} che tra l'altro forniscono anche metodi per controllare le intersezioni tra poligoni. 
	...
	\item reflection, stream e lambda nella classe \texttt{EntityImpl}: \url{https://github.com/LorenzoPrati/OOP22-dim-hol/blob/5d2b44fc5bf5fa50b6f0215695d2a5bac89cab85/src/main/java/dimhol/entity/EntityImpl.java#L61-L89}
	\item stream e lambda utilizzati anche in \texttt{RemoveEntityEvent} e in \texttt{WorldImpl} e nel metodo template di \texttt{AbstractSystem}, e in minima parte in altri punti del codice
