...
\paragraph*{Note:}
Sono consapevole che avrei potuto costruire un Menu tutuorial più funzionale, ad esempio un tutorial interattivo, ma dato il nostro dominio di 
gioco piuttosto semplice, ho valutato che bastassero pochi e semplici metodi per la 
visualizzazione a schermo dei comandi e delle regole di gioco.
...
Inoltre, e' possibile attivare una \textbf{Modalita' DEBUG}. Per farlo, andare in \textit{HOME} $\rightarrow$ \textit{OPTIONS} $\rightarrow$ spuntare il check \textit{DEBUG MODE}. Questa modalita' puo' essere utile, ad esempio ai docenti per motivi di testing, per visitare facilmente tutti i tipi le stanze e arrivare molto velocemente al boss.
